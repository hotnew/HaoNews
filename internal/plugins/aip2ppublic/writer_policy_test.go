...
func TestApplyWriterPolicyOnlyKeepsReadWriteOrigins(t *testing.T) {
	...
	postAllowed := Bundle{
		...
		Message: Message{
			...
			Extensions: map[string]any{"project": "aip2p.public", "topics": []any{"all", "world"}},
		},
	}
	postReadOnly := Bundle{
	...
	index := buildIndex([]Bundle{postAllowed, postReadOnly, replyReadOnly}, "aip2p.public")
	policy := WriterPolicy{
...
func TestApplyWriterPolicyWhitelistAcceptsOnlyExplicitWriters(t *testing.T) {
	...
	index := buildIndex([]Bundle{postAllowed, postOther}, "aip2p.public")
	policy := WriterPolicy{
...
func TestApplyWriterPolicyAllModeKeepsSignedReadOnlyWritersUnlessBlocked(t *testing.T) {
	...
	index := buildIndex([]Bundle{postReadOnly, postBlocked}, "aip2p.public")
	policy := WriterPolicy{
...
func TestApplyWriterPolicyAllModeRejectsUnsignedByDefault(t *testing.T) {
	...
	index := buildIndex([]Bundle{unsignedPost, noKeyPost}, "aip2p.public")
	policy := WriterPolicy{
		SyncMode:          WriterSyncModeAll,
		AllowUnsigned:     false,
		DefaultCapability: WriterCapabilityReadWrite,
...
func TestApplyWriterPolicyDelegationUsesParentCapability(t *testing.T) {
	...
	index := buildIndex([]Bundle{post}, "aip2p.public")
	policy := WriterPolicy{
	...
	filtered := ApplyWriterPolicyWithDelegations(index, "aip2p.public", policy, store)
	if len(filtered.Posts) != 1 {
		t.Fatalf("posts len = %d, want 1", len(filtered.Posts))
	}
