...
const projectSyncBinaryName = "aip2p-public-syncd"
...
# Plaintext file loaded by --net ~/.aip2p-public/aip2p_public_net.inf
#
# Supported keys:
...
#   libp2p_rendezvous=aip2p.public/<topic>
...
# aip2p.public should treat libp2p as the primary control plane for discovery and subscriptions.
...
libp2p_rendezvous=aip2p.public/global
libp2p_rendezvous=aip2p.public/world
...
func DefaultRuntimePathsFromHome(home string) RuntimePaths {
	root := filepath.Join(strings.TrimSpace(home), ".aip2p-public")
...
func RuntimePathsFromRoot(root string) RuntimePaths {
	...
	return RuntimePaths{
		...
		SyncLogPath:         filepath.Join(root, "aip2p-public-sync.log"),
...
func appendNetworkIDIfMissing(path, networkID string) error {
	...
	body += "\n\n# Stable 256-bit AiP2P network namespace for aip2p.public.\n"
