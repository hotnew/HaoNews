...
func TestDefaultRuntimePathsFromHome(t *testing.T) {
	...
	if paths.Root != "/tmp/example-home/.aip2p-public" {
		t.Fatalf("root = %q", paths.Root)
	}
	if paths.StoreRoot != "/tmp/example-home/.aip2p-public/aip2p/.aip2p" {
		t.Fatalf("store = %q", paths.StoreRoot)
	}
	if paths.IdentitiesRoot != "/tmp/example-home/.aip2p-public/identities" {
		t.Fatalf("identities = %q", paths.IdentitiesRoot)
	}
	if paths.DelegationsRoot != "/tmp/example-home/.aip2p-public/delegations" {
		t.Fatalf("delegations = %q", paths.DelegationsRoot)
	}
	if paths.RevocationsRoot != "/tmp/example-home/.aip2p-public/revocations" {
		t.Fatalf("revocations = %q", paths.RevocationsRoot)
	}
	if paths.ArchiveRoot != "/tmp/example-home/.aip2p-public/archive" {
		t.Fatalf("archive = %q", paths.ArchiveRoot)
	}
	if paths.RulesPath != "/tmp/example-home/.aip2p-public/subscriptions.json" {
		t.Fatalf("rules = %q", paths.RulesPath)
	}
	if paths.WriterPolicyPath != "/tmp/example-home/.aip2p-public/writer_policy.json" {
		t.Fatalf("writer policy = %q", paths.WriterPolicyPath)
	}
	if paths.WriterWhitelistPath != "/tmp/example-home/.aip2p-public/WriterWhitelist.inf" {
		t.Fatalf("writer whitelist = %q", paths.WriterWhitelistPath)
	}
	if paths.WriterBlacklistPath != "/tmp/example-home/.aip2p-public/WriterBlacklist.inf" {
		t.Fatalf("writer blacklist = %q", paths.WriterBlacklistPath)
	}
	if paths.NetPath != "/tmp/example-home/.aip2p-public/aip2p_public_net.inf" {
		t.Fatalf("net = %q", paths.NetPath)
	}
	if paths.TrackerPath != "/tmp/example-home/.aip2p-public/Trackerlist.inf" {
...
func TestEnsureRuntimeLayoutCreatesDefaultConfigFiles(t *testing.T) {
	...
	netPath := filepath.Join(root, "aip2p_public_net.inf")
	...
	if !strings.Contains(netText, "network_id="+latestOrgNetworkID) {
		t.Fatalf("missing aip2p.public network id in net config: %q", netText)
	}
...
func TestEnsureRuntimeLayoutAppendsLatestNetworkIDToExistingNetConfig(t *testing.T) {
	...
	if err := os.WriteFile(netPath, []byte("libp2p_rendezvous=aip2p.public/global\n"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	if err := ensureRuntimeLayout(store, archive, rules, writerPolicy, netPath); err != nil {
		t.Fatalf("ensureRuntimeLayout() error = %v", err)
