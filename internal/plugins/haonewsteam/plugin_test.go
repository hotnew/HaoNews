...
func TestPluginBuildServesTeamDetailAndAPI(t *testing.T) {
  ...
  "system_note_roles": ["owner"]
	...
	if !strings.Contains(rec.Body.String(), "Project Beta") || !strings.Contains(rec.Body.String(), "成员") || !strings.Contains(rec.Body.String(), "Team Policy") || !strings.Contains(rec.Body.String(), "最近消息") || !strings.Contains(rec.Body.String(), "Team Beta decided to keep Team separate from Live.") || !strings.Contains(rec.Body.String(), "最近任务") || !strings.Contains(rec.Body.String(), "Implement TeamTask") {
  ...
  "labels": ["rollout", "beta"]
	...
	if !strings.Contains(taskCreateRec.Body.String(), "\"scope\": \"team-task\"") || !strings.Contains(taskCreateRec.Body.String(), "Prepare beta rollout") {
  ...
  "labels": ["rollout", "beta", "urgent"]
	...
	if !strings.Contains(taskUpdateRec.Body.String(), "Prepare beta rollout updated") || !strings.Contains(taskUpdateRec.Body.String(), "\"doing\"") {
	...
	if !strings.Contains(taskPageRec.Body.String(), "Prepare beta rollout updated") || !strings.Contains(taskPageRec.Body.String(), "保存任务") {
  ...
  "channel_id": "research"
	...
	if !strings.Contains(artifactsPageRec.Body.String(), "Team Beta Summary") || !strings.Contains(artifactsPageRec.Body.String(), "/teams/project-beta/artifacts/artifact-beta-1") {
	...
	if !strings.Contains(artifactPageRec.Body.String(), "Long-form notes stay inside Team Artifact.") {
  ...
  "labels": ["weekly", "beta"]
	...
	if !strings.Contains(artifactPageRec.Body.String(), "Team Beta Summary Updated") || !strings.Contains(artifactPageRec.Body.String(), "保存产物") {
	...
	if !strings.Contains(rec.Body.String(), "最近变更") || !strings.Contains(rec.Body.String(), "删除 Team Artifact") {
